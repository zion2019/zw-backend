...
@Repository
public abstract class ZWMongoBasicRep<M extends BaseEntity,Q extends BaseDaoQueryCondition> implements ZWDao<M>, ZWDaoExtendQuery<Q> {
    ...
    public List<M> condition(M condition,Q extendCondition) {
    ...
    private  Query getQuery(M condition){
        return getQuery(condition,null);
    }
    private  Query getQuery(M condition,Q extendCondition){
        ...
        // 拓展query参数
        appendQuery(query,extendCondition);
